...
if len(sys.argv) != 3:
    print(f"ERROR: Wrong usage!\nCorrect one: python3 {sys.argv[0]} 'data.csv' fold_num ")
    exit(1)

FOLD = int(sys.argv[2]) - 1
...
features, labels = load_images_from_csv(sys.argv[1])

print("Finished Reading Data!")

skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=15)
for i, (train_index, test_index) in enumerate(skf.split(features, np.argmax(labels, axis=1))):
  if i!=FOLD:
    continue
